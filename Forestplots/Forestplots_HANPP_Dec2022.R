###-----------------------------------------------------------------------------------------###
### TITLE:       FORESTPLOTS HANPP SCENARIOS                                                ###
...
#SSP2.6_2100
cohorts_SSP2.6_2100_HANPP <- SSP2.6_2100_list[[2]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
  ...
  ungroup() %>%   as.data.frame()
#SSP8.5_2100
cohorts_SSP8.5_2100_HANPP <- SSP8.5_2100_list[[2]]$time_line_cohorts %>% filter(Month > 2280) %>% rowwise() %>% 
  dplyr::mutate(BiomassSum = sum(c(Biomass_FG_0,Biomass_FG_1,Biomass_FG_2,Biomass_FG_3,Biomass_FG_4,Biomass_FG_5,Biomass_FG_6,Biomass_FG_7,Biomass_FG_8)),
...
Europe <- ggplot() +
  ...
  scale_x_continuous(breaks = seq(-100,100,5),limits=c(-20,15))+
...
#export df as csv 
write.csv(HANPP,regionpath %+% "HANPP_effect.csv")
...
annotate_figure(a, top = text_grob("Effect of Current Land-Use Intensity", 
                                   face = "bold", size = 14),
...
ggsave(regionpath %+% "change_HANPP.pdf",width=297,height=210,units="mm")
...
Regions_HANPP <- ggplot() +
  ...
  scale_shape_manual(values = c("circle","diamond"),guide="none")+
  labs(title="Current Land−Use Intensity", x='Effect Size (% Change)', y = "Regions",col="Climate Scenario",trans="reverse")+
  geom_vline(xintercept=0, color='black', linetype='dashed') +
  theme_classic()+
  theme(axis.text.x = element_text(size=12))+
...
save.image(outpath %+% "Forestplot_HANPP_Image.Rdata")
